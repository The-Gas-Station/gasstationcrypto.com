...
import { isLocalChain } from '../helpers/chains';
...
export function LocalMulticallProvider({ children }: LocalMulticallProps) {
  ...
  const [localMulticallState, setLocalMulticallState] = useState(
    LocalMulticallState.Unknown,
  );
  ...
  if (
    ...
    (localMulticallState === LocalMulticallState.Deployed &&
      awaitingMulticallBlock)
  ) {
